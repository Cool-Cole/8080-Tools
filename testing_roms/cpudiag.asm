...
AIMM:	CPI	0	;A=F5H,C=0,Z=0
	...
	; FROM COLE: EVERYTHING ABOVE IS TESTING AND GOOD!!
CPIE:	CALL	CPUER
...
SUII:	SUI	00CH	;A=FFH,C=0
	...
	JZ	SBII	;TEST "SUI"
	CALL	CPUER
SBII:	SBI	0F1H	;A=F0H-0F1H-CARRY(0)=FFH,C=1 ; GOOD
	SBI	00EH	;A=FFH-OEH-CARRY(1)=F0H,C=0
	CPI	0F0H
	JZ	ANII	;TEST "SBI"
	CALL	CPUER
ANII:	ANI	055H	;A=F0H<AND>55H=50H,C=0,P=1,S=0,Z=0 ; GOOD
	CPI	050H
	JZ	ORII	;TEST "ANI"
	CALL	CPUER
ORII:	ORI	03AH	;A=50H<OR>3AH=7AH,C=0,P=0,S=0,Z=0 ; GOOD
	CPI	07AH
	JZ	XRII	;TEST "ORI"
	CALL	CPUER
XRII:	XRI	00FH	;A=7AH<XOR>0FH=75H,C=0,P=0,S=0,Z=0 ; GOOD
	CPI	075H
	JZ	C010	;TEST "XRI"
	CALL	CPUER
;
...
;
C010:	ANI	000H	;A=0,C=0,P=1,S=0,Z=1 ; GOOD
	CC	CPUER	;TEST "CC"
	CPO	CPUER	;TEST "CPO"
	CM	CPUER	;TEST "CM"
	CNZ	CPUER	;TEST "CNZ"
	...
	CALL	CPUER
C020:	SUI	077H	;A=89H,C=1,P=0,S=1,Z=0 ; GOOD
	CNC	CPUER	;TEST "CNC"
	CPE	CPUER	;TEST "CPE"
	CP	CPUER	;TEST "CP"
	CZ	CPUER	;TEST "CZ"
...
;  ------- COLE: TESTS PASS FOR ALL ABOVE THIS LINE ------
;
;
...
;
	...
	ADD	A
	CPI	0F0H
	CNZ	CPUER	;TEST "ADD" B,C,D,E,H,L,A ; COLE: PASS
	SUB	B
	SUB	C
	SUB	D
	SUB	E
	...
	SUB	L
	CPI	078H
	CNZ	CPUER	;TEST "SUB" B,C,D,E,H,L ; COLE: PASS
	SUB	A
	CNZ	CPUER	;TEST "SUB" A ; COLE: PASS
	MVI	A,080H
	ADD	A
	MVI	B,001H
	MVI	C,002H
	...
	ADC	A
	CPI	037H
	CNZ	CPUER	;TEST "ADC" B,C,D,E,H,L,A ; COLE: PASS
	MVI	A,080H
	ADD	A
	MVI	B,001H
	SBB	B
	...
	SBB	L
	CPI	0E0H
	CNZ	CPUER	;TEST "SBB" B,C,D,E,H,L ; COLE: PASS
	MVI	A,080H
	ADD	A
	SBB	A
	CPI	0FFH
	CNZ	CPUER	;TEST "SBB" A ;; COLE: PASS
	MVI	A,0FFH
	MVI	B,0FEH
	MVI	C,0FCH
	MVI	D,0EFH
	...
	ANA	A
	CPI	024H
	CNZ	CPUER	;TEST "ANA" B,C,D,E,H,L,A ; COLE: PASS
	XRA	A
	MVI	B,001H
	MVI	C,002H
	MVI	D,004H
	...
	ORA	A
	CPI	03FH
	CNZ	CPUER	;TEST "ORA" B,C,D,E,H,L,A ; COLE: PASS
	MVI	A,000H
	MVI	H,08FH
	MVI	L,04FH
	XRA	B
	...
	XRA	L
	CPI	0CFH
	CNZ	CPUER	;TEST "XRA" B,C,D,E,H,L ; COLE: PASS
	XRA	A
	CNZ	CPUER	;TEST "XRA" A ; COLE: PASS
	MVI	B,044H
	MVI	C,045H
	MVI	D,046H
	MVI	E,047H
	...
	MVI	A,044H
	CMP	B
	CNZ	CPUER	;TEST "MOV" M,B AND B,M ; COLE: PASS
	MOV	M,D
	MVI	D,000H
	MOV	D,M
	MVI	A,046H
	CMP	D
	CNZ	CPUER	;TEST "MOV" M,D AND D,M ; COLE: PASS
	MOV	M,E
	MVI	E,000H
	MOV	E,M
	MVI	A,047H
	CMP	E
	CNZ	CPUER	;TEST "MOV" M,E AND E,M ; COLE: PASS
	MOV	M,H
	MVI	H,(TEMP0 / 0FFH)
	...
	MVI	A,(TEMP0 / 0FFH)
	CMP	H
	CNZ	CPUER	;TEST "MOV" M,H AND H,M ; COLE: PASS
	MOV	M,L
	MVI	H,(TEMP0 / 0FFH)
	MVI	L,(TEMP0 AND 0FFH)
	MOV	L,M
	...
	CMP	L
	CNZ	CPUER	;TEST "MOV" M,L AND L,M ; COLE: PASS
	MVI	H,(TEMP0 / 0FFH)
	MVI	L,(TEMP0 AND 0FFH)
	MVI	A,032H
	MOV	M,A
	CMP	M
	CNZ	CPUER	;TEST "MOV" M,A ; COLE: PASS
	ADD	M
	CPI	064H
	CNZ	CPUER	;TEST "ADD" M ; COLE: PASS
	XRA	A
	MOV	A,M
	CPI	032H
	CNZ	CPUER	;TEST "MOV" A,M ; COLE: PASS
	MVI	H,(TEMP0 / 0FFH)
	MVI	L,(TEMP0 AND 0FFH)
	MOV	A,M
	SUB	M
	CNZ	CPUER	;TEST "SUB" M ; ; COLE: PASS
	MVI	A,080H
	ADD	A
	ADC	M
	CPI	033H
	CNZ	CPUER	;TEST "ADC" M ; ; COLE: PASS
	MVI	A,080H
	ADD	A
	SBB	M
	CPI	0CDH
	CNZ	CPUER	;TEST "SBB" M ; COLE PASS
	ANA	M
	...
	LDA	TEMP0
	CPI	0FFH
	CNZ	CPUER	;TEST "LDA" AND "STA" ; Passes test GOOD Cole
	LHLD	TEMPP
	SHLD	TEMP0
	LDA	TEMPP
	MOV	B,A
	LDA	TEMP0
	CMP	B
	CNZ	CPUER	;TEST "LHLD" AND "SHLD" ; THIS IS NOW GOOD AND PASSING!!!
	LDA	TEMPP+1
	MOV	B,A
	LDA	TEMP0+1
	CMP	B
	CNZ	CPUER	;TEST "LHLD" AND "SHLD" ; PASS
	MVI	A,0AAH
	STA	TEMP0
	MOV	B,H
	MOV	C,L
	...
	LDAX	B
	CPI	0AAH
	CNZ	CPUER	;TEST "LDAX" B; PASS
	INR	A
	STAX	B
	LDA	TEMP0
	CPI	0ABH
	CNZ	CPUER	;TEST "STAX" B; PASS
	MVI	A,077H
	STA	TEMP0
	LHLD	TEMPP
	LXI	D,00000H
	...
	LDAX	D
	CPI	077H
	CNZ	CPUER	;TEST "LDAX" D AND "XCHG"; PASS
	XRA	A
	ADD	H
	ADD	L
	CNZ	CPUER	;TEST "XCHG"; PASS
	MVI	A,0CCH
	STAX	D
	LDA	TEMP0
	CPI	0CCH
	...
	LDA	TEMP0
	CPI	0CCH
	CNZ	CPUER	;TEST "STAX" D; PASS
	LXI	H,07777H
	DAD	H
	MVI	A,0EEH
	CMP	H
	CNZ	CPUER	;TEST "DAD" H; PASS
	CMP	L
	CNZ	CPUER	;TEST "DAD" H; PASS
	LXI	H,05555H
	LXI	B,0FFFFH
	DAD	B
	MVI	A,055H
	CNC	CPUER	;TEST "DAD" B; PASS
	CMP	H
	CNZ	CPUER	;TEST "DAD" B; PASS
	MVI	A,054H
	CMP	L
	CNZ	CPUER	;TEST "DAD" B; PASS
	LXI	H,0AAAAH
	...
	MVI	A,0DDH
	CMP	H
	CNZ	CPUER	;TEST "DAD" D; PASS
	CMP	L
	CNZ	CPUER	;TEST "DAD" B; PASS
	STC
	CNC	CPUER	;TEST "STC"; PASS
	CMC
	CC	CPUER	;TEST "CMC
	MVI	A,0AAH
	CMA	
	...
	MVI	A,042H
	RLC
	CC	CPUER	;TEST "RLC" FOR RE-SET CARRY; PASS
	RLC
	CNC	CPUER	;TEST "RLC" FOR SET CARRY; PASS
	CPI	009H
	CNZ	CPUER	;TEST "RLC" FOR ROTATION; PASS
	RRC
	CNC	CPUER	;TEST "RRC" FOR SET CARRY; PASS
	RRC
	CPI	042H
	CNZ	CPUER	;TEST "RRC" FOR ROTATION; PASS
	RAL
	RAL
	CNC	CPUER	;TEST "RAL" FOR SET CARRY; PASS
	CPI	008H
	CNZ	CPUER	;TEST "RAL" FOR ROTATION; PASS
	RAR
	RAR
	CC	CPUER	;TEST "RAR" FOR RE-SET CARRY; PASS
	CPI	002H
	CNZ	CPUER	;TEST "RAR" FOR ROTATION; PASS
	LXI	B,01234H
	LXI	D,0AAAAH
	LXI	H,05555H
	XRA	A
	...
	POP	D
	POP	B
	CC	CPUER	;TEST "PUSH PSW" AND "POP PSW"; PASS
	CNZ	CPUER	;TEST "PUSH PSW" AND "POP PSW"; PASS
	CPO	CPUER	;TEST "PUSH PSW" AND "POP PSW"; PASS
	CM	CPUER	;TEST "PUSH PSW" AND "POP PSW"; PASS
	MVI	A,012H
	CMP	B
	CNZ	CPUER	;TEST "PUSH B" AND "POP B"; PASS
	MVI	A,034H
	CMP	C
	CNZ	CPUER	;TEST "PUSH B" AND "POP B"; PASS
	MVI	A,0AAH
	CMP	D
	CNZ	CPUER	;TEST "PUSH D" AND "POP D"; PASS
	CMP	E
	CNZ	CPUER	;TEST "PUSH D" AND "POP D"; PASS
	MVI	A,055H
	CMP	H
	CNZ	CPUER	;TEST "PUSH H" AND "POP H"; PASS
	CMP	L
	CNZ	CPUER	;TEST "PUSH H" AND "POP H"; PASS
	LXI	H,00000H
	DAD	SP
	...
	POP	B
	CMP	B
	CNZ	CPUER	;TEST "LXI","DAD","INX",AND "DCX" SP; PASS
	CMA
	CMP	C
	CNZ	CPUER	;TEST "LXI","DAD","INX", AND "DCX" SP; PASS
	LXI	H,TEMP4
	...
	LDA	TEMP3
	CPI	077H
	CNZ	CPUER	;TEST "SPHL" AND "XTHL"; PASS
	LDA	TEMP2
	CPI	033H
	CNZ	CPUER	;TEST "SPHL" AND "XTHL"; PASS
	MVI	A,055H
